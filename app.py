"""
=============================================================
  AQI 7-Day Prediction System — Streamlit App (app.py)
  FIXED: Feature mismatch bug corrected. All 17 features
         now match exactly between training and inference.
  ...
  Run locally:
    pip install streamlit scikit-learn pandas numpy joblib requests
...
CSV_PATH       = p("aqi.csv")
MODEL_BEST     = p("best_model.pkl")
MODEL_SCALER   = p("scaler.pkl")
MODEL_LE_STATE = p("label_encoder_state.pkl")
MODEL_LE_AREA  = p("label_encoder_area.pkl")
MODEL_LE_POLL  = p("label_encoder_pollutant.pkl")

# EXACT feature list — must match training order
FEATURES = [
...
OPENWEATHER_API_KEY = "YOUR_API_KEY"   # ← optional OWM key
PREDICTION_DAYS     = 7
...
def load_artifacts():
    ...
    le_state = joblib.load(MODEL_LE_STATE)
    le_area  = joblib.load(MODEL_LE_AREA)
    le_poll  = joblib.load(MODEL_LE_POLL)
...
# CHECK FILES
# ─────────────────────────────────────────────
missing = [f for f, fp in [
    ("aqi.csv", CSV_PATH), ("best_model.pkl", MODEL_BEST),
    ("scaler.pkl", MODEL_SCALER), ("label_encoder_state.pkl", MODEL_LE_STATE),
    ("label_encoder_area.pkl", MODEL_LE_AREA), ("label_encoder_pollutant.pkl", MODEL_LE_POLL),
...
if missing:
    ...
    for f in missing:
        st.markdown(f"- ❌ `{f}`")
    st.markdown("""
    ### Fix: Place all these files in the **same folder** as `app.py`:
    ```
    your_repo/
    ├── app.py
    ├── requirements.txt
...
# ─────────────────────────────────────────────
# PREDICTION — 17 features, iterative rollout
# ─────────────────────────────────────────────
def predict_7days(df, model, scaler, le_state, le_area, le_poll,
    ...
    # Keep full rolling history (up to 30 values)
    ...
    for d in range(1, PREDICTION_DAYS + 1):
        ...
        # ── Build feature vector (must be 17 in FEATURES order) ────
        feat_vec = [
            fd.year,                         # year
            fd.month,                        # month
            fd.weekday(),                    # day_of_week
            fd.timetuple().tm_yday,          # day_of_year
            (fd.month - 1) // 3 + 1,        # quarter
            int(fd.weekday() >= 5),          # is_weekend
            season_of(fd.month),             # season
            st_enc,                          # state_enc
            ar_enc,                          # area_enc
            po_enc,                          # poll_enc
            monitoring,                      # monitoring_stations
            lag1,                            # aqi_lag_1
            lag7,                            # aqi_lag_7
            lag30,                           # aqi_lag_30
            roll7,                           # aqi_roll_7d
        ...
        # Safety check
        assert len(feat_vec) == len(FEATURES), \
            f"Feature count mismatch: {len(feat_vec)} != {len(FEATURES)}"

        X_sc     = scaler.transform([feat_vec])
        pred_aqi = float(np.clip(model.predict(X_sc)[0], 0, 500))

        # ── Blend with OWM API if available ────────────────────────
        ...
        if api_daily is not None and d <= len(api_daily):
            ...
            pm25 = round(float(row_fc["pm2_5"]), 1)
            pm10 = round(float(row_fc["pm10"]),  1)
            no2  = round(float(row_fc["no2"]),   1)
            o3   = round(float(row_fc["o3"]),    1)
            so2  = round(float(row_fc["so2"]),   1)
            co   = round(float(row_fc["co"]),    1)
            nh3  = round(float(row_fc["nh3"]),   1)
            no   = round(float(row_fc["no"]),    1)

        pred_aqi = round(pred_aqi, 1)

        # ── Feed prediction back as next lag ───────────────────────
        ...
        results.append({
            ...
            "Day":  fd.strftime("%A"),
            "AQI":  pred_aqi, "Category": cat, "Color": aqi_color(pred_aqi),
            "PM2.5": pm25, "PM10": pm10,
            "NO2": no2, "O3": o3, "SO2": so2, "CO": co, "NH3": nh3, "NO": no,
        })
...
with st.expander("ℹ️ Model Info", expanded=False):
    ...
    c1.metric("Model",         "RandomForest")
    c2.metric("R² Score",      "0.7981")
    c3.metric("MAE",           "20.94 AQI pts")
...
if predict_btn:
    if not state or not area:
        ...
        if err:
            ...
            st.success(f"✅ 7-Day Forecast for **{area}, {state}**")
            ...
            for i, day in enumerate(forecast):
                with ccols[i]:
                    ...
                    <div style="background:{day['Color']}22;border:2px solid {day['Color']};
                      ...
                      <div style="font-size:0.72rem;font-weight:700;color:{day['Color']}">{day['Day'][:3].upper()}</div>
                      <div style="font-size:0.65rem;color:{day['Color']};opacity:0.85">{day['Date']}</div>
            ...
            # ── Pollutants Table ─────────────────────
            st.subheader("🧪 Pollutants Detail")
            ...
            if has_poll:
                st.dataframe(pd.DataFrame([{
                    ...
                    "SO₂ μg/m³": d["SO2"], "CO μg/m³": d["CO"], "NH₃ μg/m³": d["NH3"],
                ...
                st.info(
                    "💡 Add your OpenWeatherMap API key in `app.py` to see live pollutant data.  \n"
            ...
            st.download_button(
                ...
                file_name=f"aqi_forecast_{area.replace(' ','_')}_{datetime.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
            )
